...
class OLSResults:
    ...
    ssr: torch.Tensor            # (R,) model objective in the estimation metric
    _nobs: int
    df_resid: int
    df_model: int
    has_const: bool = True
    ...
    def predict(self, X_new: torch.Tensor, *, params: str = "replications") -> torch.Tensor:
        ...
        Xn = X_new if isinstance(X_new, torch.Tensor) else torch.as_tensor(X_new)
        ...
        if Xn.ndim == 2:
            ...
            if params == "replications":
                Xb = Xn.unsqueeze(0).expand(self.R, n_new, k)
                return (Xb @ self.params.unsqueeze(-1)).squeeze(-1)

            if params == "mean":
                beta = self.params.mean(dim=0)
                return (Xn @ beta)

            if params == "median":
                beta = self.params.median(dim=0).values
                return (Xn @ beta)
        ...
        if Xn.ndim == 3:
            ...
            if params == "replications":
                ...
                return (Xn @ self.params.unsqueeze(-1)).squeeze(-1)

            if params == "mean":
                beta = self.params.mean(dim=0)
                return (Xn @ beta.view(1, k, 1)).squeeze(-1)
    ...
    def get_fitted(self, X: torch.Tensor, *, params: str = "replications") -> torch.Tensor:
        """Compute fitted values on-the-fly given a design matrix X."""
        return self.predict(X, params=params)

    def get_resid(self, X: torch.Tensor, y: torch.Tensor, *, params: str = "replications") -> torch.Tensor:
        ...
        if Xn.ndim == 2:
            ...
            if params == "replications":
                if yn.ndim == 1:
                    ...
                    yb = yn.view(1, n).expand(self.R, n)
                    return yb - yhat
                if yn.ndim == 2:
    ...
    def durbin_watson_on(self, X, y, *, params: str = "replications") -> torch.Tensor:
    ...
    def _resid_for_plot(self, X=None, y=None) -> torch.Tensor:
        """Get residuals for plotting/diagnostics. Returns (R,n)."""
    ...
    def qq_resid_on(self, X=None, y=None, *, r: int = 0, standardize: bool = True) -> tuple[np.ndarray, np.ndarray]:
        """Compute Normal Q-Q plot data for residuals of replication r."""
        e = self._resid_for_plot(X=X, y=y)
        R, n = e.shape
        if not (0 <= r < R):
            raise ValueError(f"r must be in [0,{R-1}]. Got {r}.")

        er = e[r].detach().cpu().numpy().astype(float)

        if standardize:
            m = float(er.mean())
            s = float(er.std(ddof=1)) if n > 1 else float(er.std())
            er = (er - m) / s if s > 0 else (er - m)
    ...
    def acf_resid_on(self, X=None, y=None, *, r: int = 0, nlags: int = 20, demean: bool = True) -> np.ndarray:
    ...
    def plot(
        ...
        *,
        ...
        """Lightweight plotting utilities. Uses on-the-fly residuals if needed."""
        ...
        if kind == "params":
            ...
            ci = self.conf_int(alpha=alpha, use_t=self.use_t)
    ...
    def conf_int(self, alpha: float = 0.05, use_t: Optional[bool] = None) -> torch.Tensor:
        """Confidence intervals per replication. Output shape: (R, k, 2)."""
        if use_t is None:
            use_t = self.use_t

        q = 1.0 - alpha / 2.0
        crit = float(st.t.ppf(q, df=self.df_resid)) if use_t else float(st.norm.ppf(q))
    ...
    def pvalues_beta0(self, beta0: torch.Tensor, use_t: Optional[bool] = None) -> torch.Tensor:
        """Two-sided p-values for H0: beta = beta0 (componentwise)."""
        stat = self.stat_beta0(beta0, use_t=use_t)
        return self._pvalues_from_stat(stat, use_t=use_t)

    def reject_beta0(self, beta0: torch.Tensor, alpha: float = 0.05, use_t: Optional[bool] = None) -> torch.Tensor:
    ...
    def tss(self) -> Optional[torch.Tensor]:
        ...
        Requires y to be stored, unless model provided a custom tss in extras.
        """
        ex = self._ex()
        if "tss" in ex and isinstance(ex["tss"], torch.Tensor):
            return ex["tss"]
        if "tss_weighted" in ex and isinstance(ex["tss_weighted"], torch.Tensor):
            return ex["tss_weighted"]
    ...
    def rsquared(self) -> Optional[torch.Tensor]:
        ex = self._ex()
        if "rsquared" in ex and isinstance(ex["rsquared"], torch.Tensor):
            return ex["rsquared"]
        if "rsquared_weighted" in ex and isinstance(ex["rsquared_weighted"], torch.Tensor):
            return ex["rsquared_weighted"]
    ...
    def adj_rsquared(self) -> Optional[torch.Tensor]:
        ex = self._ex()
        if "adj_rsquared" in ex and isinstance(ex["adj_rsquared"], torch.Tensor):
            return ex["adj_rsquared"]
        if "adj_rsquared_weighted" in ex and isinstance(ex["adj_rsquared_weighted"], torch.Tensor):
            return ex["adj_rsquared_weighted"]
    ...
    def llf(self) -> torch.Tensor:
        """
        Log-likelihood.
        - Default: homoskedastic Gaussian (statsmodels-style): sigma2_mle = SSR / n.
        - If extras['llf'] exists, returns that (useful for GLS/WLS).
        """
        ex = self._ex()
        if "llf" in ex and isinstance(ex["llf"], torch.Tensor):
            return ex["llf"]
    ...
    def aic(self) -> torch.Tensor:
        ex = self._ex()
        if "aic" in ex and isinstance(ex["aic"], torch.Tensor):
            return ex["aic"]
    ...
    def bic(self) -> torch.Tensor:
        ex = self._ex()
        if "bic" in ex and isinstance(ex["bic"], torch.Tensor):
            return ex["bic"]
        return -2.0 * self.llf + np.log(float(self.nobs)) * float(self.k)

    @property
    def durbin_watson(self) -> Optional[torch.Tensor]:
        """Durbin-Watson per replication (R,). Requires resid to be stored."""
        if self.resid is None:
            return None
        if self.nobs < 2:
            return None
        de = self.resid[:, 1:] - self.resid[:, :-1]
        denom = (self.resid ** 2).sum(dim=1)
        return (de ** 2).sum(dim=1) / denom
