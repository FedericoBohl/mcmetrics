...
from mcmetrics.exceptions import SingularMatrixError, ShapeError
...
def safe_cholesky(
    ...
    *,
    ...
    k = A0.shape[-1]
    I = torch.eye(k, device=A0.device, dtype=A0.dtype)

    last_err: Optional[Exception] = None
    for t in range(max_tries):
        try:
            Aj = A0 + (jitter * (10.0 ** t)) * I if jitter > 0.0 else A0
